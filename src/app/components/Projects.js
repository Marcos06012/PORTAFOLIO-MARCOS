    ...
    return (
        ...
        >
            <div className="max-w-7xl w-full flex flex-col items-center justify-center z-10 px-4 md:px-8">
                ...
                {/* MODO MÓVIL: Swiper interactivo optimizado y responsive */}
                <motion.div
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                    <Swiper
                        ...
                        className="mySwiper relative w-full pb-10"
                    >
                        {projectItems.map((project, idx) => (
                            <SwiperSlide key={idx}>
                                <div className="group bg-gradient-to-b from-gray-950 to-gray-900/90 border border-white/5 rounded-[2.2rem] min-h-[460px] flex flex-col justify-between p-6 text-left shadow-xl relative overflow-hidden">
                                    
                                    {/* Cabecera Móvil */}
                                    <div className="w-full flex justify-between items-start z-10 gap-2 mb-3">
                                        ...
                                        <div className="flex items-center gap-2 z-20">
                                            {project.github && project.github !== "#" && (
                                                <a href={project.github} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 active:bg-gray-800 active:text-white transition-colors">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" /><path d="M9 18c-4.51 2-5-2-7-2" /></svg>
                                                </a>
                                            )}
                                            {project.demo && project.demo !== "#" && (
                                                <a href={project.demo} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 active:bg-emerald-500 active:text-white transition-colors">
                                    ...
                                    {/* Imagen en Móvil */}
                                    <div className="w-full h-44 bg-gray-950/60 border border-white/5 rounded-2xl flex items-center justify-center relative overflow-hidden mb-4">
                                        <Image src={`/img/${project.image}`} alt={project.title} width={400} height={250} className="object-cover w-full h-full rounded-2xl opacity-90" />
                                    </div>

                                    {/* Descripción Inferior Móvil */}
                                    <div className="w-full z-10 border-t border-white/5 pt-4 flex flex-col gap-2">
                                        <p className="text-xs text-gray-400 font-light text-justify leading-relaxed line-clamp-4">
                                            {project.description}
                                        </p>
                                    </div>
                                </div>
                    ...
                    {/* CORRECCIÓN: Contenedor físico para renderizar los círculos de paginación de Swiper */}
                    <div className="custom-projects-pagination flex justify-center gap-1.5 mt-2" />
                </motion.div>
